...
static char registerNames[REGISTER_QUANTITY][STR_LEN] = {
  "RIP", "RAX", "RBX", "RCX", "RDX", "RBP", "RSP", "RDI", "RSI",
...
void printGenericRegisters(uint64_t regs[REGISTER_QUANTITY]){
  for(int i = 0; i < REGISTER_QUANTITY; i++){
    printNextString(registerNames[i]); printNextChar(' '); 
    printNextHexWithPadding(regs[i]); printNextChar(' ');
