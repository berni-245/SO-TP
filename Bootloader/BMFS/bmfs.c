  ...
  if (strcasecmp(s_list, command) == 0) {
    ...
    if (filename == NULL) {
      ...
      if (argc > 4) {
        ...
        if (filesize >= 1) create(filename, filesize);
        else printf("Error: Invalid file size.\n");
      }
    }
...
int initialize(char* diskname, char* size, char* mbr, char* boot, char* kernel) {
  ...
  if (ret == 0) {
    if (diskSize < minimumDiskSize) {
      printf(
          "Error: Disk size must be at least %llu bytes (%lluMiB)\n", minimumDiskSize, minimumDiskSize / (1024 * 1024)
      );
      ret = 1;
    }
  }
  ...
  if (findfile(filename, &tempentry, &slot) == 0) {
    ...
    for (tint = 0; tint < 64; tint++) {
      ...
      if (pEntry->FileName[0] == 0x00)                    // end of directory
      {
        ...
        if (first_free_entry == -1) first_free_entry = tint; // there were no unused entires before, will use this one
        break;
    ...
    for (tint = 0; tint < num_used_entries + 1; tint++) {
      ...
      else this_file_start = pEntry->StartingBlock;
